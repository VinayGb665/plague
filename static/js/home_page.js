...
function fetch_members_and_populate_courses(){
    ...
    $.get('/mycourses',(data)=>{
        if(data){
            console.log('Course fetch reponse:',data)
            for(i=0;i<data.length;i++){

                html_str+=render_user_html(data[i])
            }
            if(i==0){
                html_str+="<center><span>No Courses yet add them by clicking this button here</span></center>"
            }
...
function render_user_html(name){
   ...
   <img src="static/images/idea.svg" class="bd-placeholder-img mr-2 rounded" width="32" height="42" xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="xMidYMid slice" focusable="false" role="img" ></img>
